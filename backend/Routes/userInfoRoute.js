...
router.post('/user', (req, res) => {
    ...
    if (!name || !age || !email || !startDate || !feesPaid || !batchNumber || name == "" || email == "" || startDate == "NaN/undefinedundefined/" || batchNumber == 0) {
        ...
        if (startDate < currentDate) {
            res.status(401).json({ "message": "Start date cannot be smaller than today's date", "message_id": "InStDa" })
            return;
        }
        //If email and age both are invalid
        else if (isEmailValid == false && (age < 18 || age > 65)) {
            res.status(401).json({ "message": "Invalid email address and invalid age", "message_id": "InEmInAg" })
            return;
        }
            ...
            .then((savedUser) => {
                ...
                if (savedUser) {
                    ...
                    if (Days_difference > 30) {
                        userCollection.updateOne({ email: email }, { $set: { startDate: currentDate } })
                            .then((updatedUser) => {
